...
// solution:------------------------------------------------------------------------------------
 public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        visited[sr][sc] = true;
        ans[sr][sc] = newColor;
        bfs(initial, ans, sr, sc, newColor, visited);
        return ans;
    }
    public static void bfs(int initial, int[][] ans, int sr, int sc, int newColor, boolean[][] visited) {
        int n = ans.length, m = ans[0].length;
        Queue<Integer> qrow = new LinkedList<>(), qcol = new LinkedList<>();
        qrow.add(sr); qcol.add(sc);
        int[] drow = {1, -1, 0, 0}, dcol = {0, 0, 1, -1};
        while (!qrow.isEmpty() && !qcol.isEmpty()) {
            int row = qrow.poll(), col = qcol.poll();
            for (int i = 0; i < 4; i++) {
                int r = row + drow[i], c = col + dcol[i];
                if (r >= 0 && r < n && c >= 0 && c < m) {
                    if (ans[r][c] == initial && !visited[r][c]) {
                        visited[r][c] = true;
                        ans[r][c] = newColor;
                        qrow.add(r); qcol.add(c);
                    }
                }
            }
        }
